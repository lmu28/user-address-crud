...
import com.example.user_address_crud.exception.AuthException;
...
public class AccountServiceImpl implements AccountService {
    ...
    public AuthResponse login(AuthRequest loginRequest) {
        try {
            ...
            throw new AuthException("Неверное имя пользователя или пароль", null);
        } catch (AuthenticationException e) {
            throw new AuthException("Ошибка аутентификации: " + e.getMessage(), null);
        }
    }
    ...
    public AuthResponse register(AuthRequest registerRequest) {
        if (accountRepository.existsByUsername(registerRequest.getUsername())) {
            throw new AuthException("Имя пользователя уже занято", "username");
        }
        ...
        // Автоматически логин после регистрации
